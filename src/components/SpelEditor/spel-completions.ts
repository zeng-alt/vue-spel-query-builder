...
export function buildEntries(
  ...
  if (authentication) {
    list.push({ label:'authentication', type:'variable', detail:'用户认证信息',
      desc:'存储当前登录用户的认证信息。', extra:'authentication.name / authentication.roles' })
  ...
  if (principal) {
    list.push({ label:'principal', type:'variable', detail:'基础变量信息',
      desc:'业务主体对象。', extra:'principal.id / principal.status' })
  ...
  ;[
    { label:'contains', detail:'包含判断', desc:'字符串或集合包含判断。', extra:"principal.name.contains('admin')" },
    { label:'startsWith', detail:'前缀匹配', desc:'字符串前缀匹配。', extra:"principal.code.startsWith('ORD')" },
    { label:'endsWith', detail:'后缀匹配', desc:'字符串后缀匹配。', extra:"principal.file.endsWith('.pdf')" },
    { label:'matches', detail:'正则匹配', desc:'正则表达式匹配。', extra:"principal.phone.matches('1[3-9]\\\\\\\\d{9}')" },
    { label:'isEmpty', detail:'判断为空', desc:'字符串或集合长度是否为 0。', extra:'principal.tags.isEmpty()' },
...
export function buildTypeMap(
  ...
  function walk(obj: any, prefix: string) {
    ...
    for (const key of Object.keys(obj)) {
      ...
      else if (typeof val === 'object') { map[fp] = 'object'; walk(val, fp) }
  ...
  if (locals) {
    for (const key of Object.keys(locals)) {
      const fp = `#${key}`; const val = locals[key]
      if (val === null || val === undefined) map[fp] = 'null'
      else if (Array.isArray(val)) map[fp] = 'array'
      else if (typeof val === 'object') { map[fp] = 'object'; walk(val, fp) }
      else map[fp] = typeof val
...
export function buildElementFields(obj: any): ElementField[] {
  ...
  return Object.keys(obj).map(key => {
    const val = obj[key]; const f: ElementField = { label: key, value: key, type: 'null' }
    ...
    } else if (typeof val === 'object') { f.type = 'object'; f.children = buildElementFields(val) }
...
export function buildArrayMeta(
  ...
  function walkArr(obj: any, prefix: string) {
    if (!obj || typeof obj !== 'object') return
    for (const key of Object.keys(obj)) {
      const fp = prefix ? `${prefix}.${key}` : key; const val = obj[key]
...
export function buildStringMethodEntries(): SpelEntry[] {
  return [
    { label:'length', type:'property', detail:'字符串长度', desc:'返回字符串的字符数量。', extra:'principal.name.length' },
    { label:'isEmpty()', type:'function', detail:'判断为空', desc:'判断长度是否为 0。', extra:'principal.name.isEmpty()' },
    ...
    { label:'substring(x)', type:'function', detail:'子串截取(1参)', desc:'从指定位置截取。', extra:'principal.name.substring(3)' },
    { label:'substring(x,y)', type:'function', detail:'子串截取(2参)', desc:'从起始截取到结束。', extra:'principal.name.substring(0, 3)' },
    { label:'replace(x,y)', type:'function', detail:'字符串替换', desc:'替换字符。', extra:"principal.name.replace('old', 'new')" },
    { label:'startsWith(x)', type:'function', detail:'前缀匹配', desc:'是否以指定前缀开头。', extra:"principal.code.startsWith('ORD')" },
    { label:'endsWith(x)', type:'function', detail:'后缀匹配', desc:'是否以指定后缀结尾。', extra:"principal.file.endsWith('.pdf')" },
    { label:'contains(x)', type:'function', detail:'包含判断', desc:'是否包含指定子串。', extra:"principal.name.contains('value')" },
    { label:'indexOf(x)', type:'function', detail:'查找索引', desc:'返回首次位置。', extra:"principal.name.indexOf('a')" },
    { label:'charAt(x)', type:'function', detail:'获取字符', desc:'返回指定位置字符。', extra:'principal.name.charAt(0)' },
  ]
}

export function buildArrayMethodEntries(am?: ArrayMeta): SpelEntry[] {
  const r: SpelEntry[] = [
    { label:'size()', type:'function', detail:'集合大小', desc:'返回数组元素数量。', extra:'...roles.size() > 0' },
    { label:'contains(x)', type:'function', detail:'包含判断', desc:'数组是否包含指定元素。', extra:"...roles.contains('admin')" },
  ]
  if (am?.elementType === 'object' && am.elementFields) {
    r.push({ label:'?[field == x]', type:'function', detail:'过滤-对象数组',
      desc:`可用字段：${am.elementFields.map(f => f.label).join(', ')}`,
      extra:`...roles.?[${am.elementFields[0]?.label ?? 'field'} == value]` })
    r.push({ label:'![field]', type:'function', detail:'投影-提取字段',
      desc:'从对象数组中提取指定字段。', extra:'...roles.![code]' })
    for (const f of am.elementFields)
      r.push({ label:`[0].${f.label}`, type:'property', detail:'数组元素字段',
        desc:`元素 ${f.label}，类型 ${f.type}`, extra:`...roles[0].${f.label}` })
  } else {
    r.push({ label:'?[#this == x]', type:'function', detail:'过滤-基本类型数组',
...
export function buildFilterFieldEntries(fields: ElementField[], prefix = ''): SpelEntry[] {
  return fields.map(f => ({
    label: f.label, type: 'property' as const,
